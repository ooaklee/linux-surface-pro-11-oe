...
| Pen | 🧪 Integration candidate | The matching pen-part-2 kernel branch exposes a private HIDRAW device to pinned upstream iptsd while iptsd touch output is disabled. Targeted ARM64 kernel objects and userspace builds pass; target-device, suspend/resume, and tool/barrel-button validation are pending. Unmodified iptsd v3.1.0 does not expose a second stylus button. The live desktop is not modified. See [ADR0067](docs/adr/adr-0067-sp11-kernel-hidraw-iptsd-pen-integration.md). |
...
# Pen part 2 test branch (build and validate before merge)
