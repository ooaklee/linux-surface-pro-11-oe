...
The collector creates a fresh, strict hand-off directory. Never collect
directly to removable storage; after success, use the linked procedure to copy
the completed child to a new directory on trusted removable storage. Treat
both copies as private, device-bound and proprietary. Do not publish them,
commit them, or include them in an ISO or release. The collector deliberately
excludes Windows Wi-Fi firmware.
